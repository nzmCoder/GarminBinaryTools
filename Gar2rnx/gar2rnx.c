...
typedef struct {double llh[3]; double xyz[3];} type_rec0x11;

typedef struct {int week; int garmin_wdays; double tow;} type_rec0x0e;

typedef struct{ float delta_pr; float f1; double pr; float f2; BYTE sv; }  
type_rec0x16;

typedef struct{ BYTE sv; BYTE elev; UINT frac; UINT db; BYTE tracked; BYTE flag; }
type_rec0x1a;

typedef struct{ float altitud; float epe[3];  UINT fix; double tow; double pos[2];
                float vel[3];  float h_ellip; UINT leap; ULONG wdays;             }
type_rec0x33;


typedef struct{ ULONG c50; BYTE uk[4]; BYTE sv; }
type_rec0x36;

 
typedef struct { ULONG c_phase; 
                 long int tracked;   //BYTE  tracked; UINT cc2; BYTE flag; 
                 UINT  delta_f;  ULONG  int_phase;  
                 double pr;  
                 ULONG  c511;   
                 UINT  db;   double tow; BYTE   sv;  }  
type_rec0x38;


typedef struct{ BYTE used; UINT db; double prange; double phase; 
                float doppler; BYTE elev; long int tracked; ULONG c511; float last36;} 
rinex_obs;         
...
type_rec0x11 process_0x11(BYTE *record) // Position
{
 ...
 memcpy(&rec.llh[0],record,8); memcpy(&rec.llh[1],record+8,8); 
 

 rec.llh[0]*=(180/WGS84_PI); rec.llh[1]*=(180/WGS84_PI); rec.llh[2]=0.0;
 ...
 if(VERBOSE)
  {
   ...
   for(i=0;i<4;i++) printf("%02x ",rec.uk[i]); printf("\n");
  }
...
type_rec0x33 process_0x33(BYTE *record)
{
 ...
 memcpy(&rec.altitud,record,4); record+=4;
 memcpy(&rec.epe,record,12); record+=12;
 memcpy(&rec.fix,record,2); record+=2;
 memcpy(&rec.tow,record,8); record+=8;
 memcpy(&rec.pos,record,16); record+=16;
 memcpy(&rec.vel,record,12); record+=12;
 memcpy(&rec.h_ellip,record,4); record+=4;
 memcpy(&rec.leap,record,2); record+=2;
 memcpy(&rec.wdays,record,4); record+=4;
...
void process_0x37(BYTE *record)
{
 ...
 memcpy(&countdown,record+8,2); TR=((float)1024.0-countdown)/16.0;
...
void process_0x1a(BYTE *record)
{
 ...
 for(k=0,rec=record;k<12;k++,rec+=8)
 {
  if (ETREX)
   {
    svid=rec[5]; elev=rec[4]; sq=*((UINT*)(rec+2));
    frac_phase=*((UINT*)(rec+0))/2048.0; trk=rec[6]; flag=rec[7];
   }
  else
   {
    svid=rec[0]; elev=rec[1]; sq=*((UINT*)(rec+4));
    frac_phase=*((UINT*)(rec+2))/2048.0; trk=rec[6]; flag=rec[7];
   }                        
  if (ONE_SAT)
  {
...
type_rec0x38 process_0x38(BYTE* record)
{
 ...
 if (ONE_SAT && ( record[36] != SELECTED_SV)) {rec.sv=record[36]; return rec;}
 ...
 if(VERBOSE)
  ...
  itow=(long)floor(rec38.tow+0.5);  if(START==-1) START=itow;
  dtow=itow-START;
 ...
 if (ONE_SAT && DIF_RECORDS)   // Differencing records is only valid when tracking one sat
  {
   ...
   if (fabs(slip)<5000)
    {
     mean_q+=rec38.db;
     mean+=(slip/nsec); sigma+=(slip*slip/(nsec*nsec)); n++;
    }
...
void collect_stats(FILE *org)
{
 ...
 do 
 {
  fread(&id,1,1,org); fread(&L,1,1,org); fread(record,1,L,org); //fseek(org,L,SEEK_CUR);    
...
void check(BYTE *record,BYTE L,BYTE type)
{
 ...
 switch(type)
 {
  case AS_BYTE: for(k=0;k<L;k++) printf("%02x ",record[k]); printf("\n"); break;
  case AS_CHAR: for(k=0;k<L;k++) printf("%c",record[k]); printf("\n"); break;
  case AS_DOUBLE: size=sizeof(double); if(L<size) return;
          for(k=0;k<=L-size;k++) printf("%3d %f\n",k,*((double*)(record+k)));
          break;
  case AS_FLOAT: size=sizeof(float); if(L<size) return;
          for(k=0;k<=L-size;k++) printf("%3d %f\n",k,*((float*)(record+k)));
          break;  
  case AS_ULONG: size=sizeof(ULONG); if(L<size) return;
          for(k=0;k<=L-size;k++) printf("%3d %lu\n",k,*((ULONG*)(record+k)));
          break;  
  case AS_UINT: size=sizeof(UINT); if(L<size) return;    
...
float *version;
{
 ...
 do
 {
  fread(&id,1,1,org); fread(&L,1,1,org); fread(record,1,L,org);
  switch (id)
   {
    case 0xff: 
      bad=0;k=4;   // Check if it is a good looking ID record 
      while(record[k++] && !bad) bad=(record[k]>=128);
      if(!bad) // Probably  a good ff record
       {
        temp=get_uint(record); *prod=temp; 
        temp=get_uint(record+2); *version=(float)temp/100;
 ...
 *prod=0; *version=0.0; strcpy(description,"Generic GPS12");
 rewind(org);
 return; 
} 
...
void get_wdays_and_tow_from_user_date(ULONG* wdays,ULONG* week_secs)   
{
 ...
 hour=(UINT)12; min=(UINT)0; sec=(UINT)0;  
 
 jd=julday(year,month,day,hour,min,(double)sec);
 tow=(ULONG)gps_time(jd,&gps_week); *week_secs=(ULONG)tow;
...
ULONG *tow;
{
 ...
 do
 {
  ...
  switch (id)  
   {
    case 0x33: 
               rec=process_0x33(record); k++; if(k>GET_THIS) break;
               fix=rec.fix;
    ...
    case 0x0e: rec0e=process_0x0e(record); found_0e=1; break;

    case 0x11: rec11=process_0x11(record); found_11=1; break;
   ...
   if (found_0e && found_11)   // Found 0x11 AND 0x0e records
    {
     ...
     for(k=0;k<3;k++) xyz[k]=rec11.llh[k]; llh2xyz(xyz,xyz);
...
FILE *fd;
{
 ...
 written=sprintf(ptr,"%6d%14c",2,32); ptr+=written;
 written=sprintf(ptr,padd("OBSERVATION FILE",buffer,20)); ptr+=written;
 written=sprintf(ptr,padd("GPS NAVSTAR",buffer,20)); ptr+=written;
 written=sprintf(ptr,padd("RINEX VERSION / TYPE",buffer,20)); ptr+=written;

 sprintf(buffer,"GAR2RNX %4.2f",VERSION);
 written=sprintf(ptr,padd(buffer,buffer,20)); ptr+=written; 
 written=sprintf(ptr,padd("Any GPS12 Owner",buffer,20)); ptr+=written; 
 time(&tt); date=ctime(&tt); 
 written=sprintf(ptr,padd(date,buffer,20)); ptr+=written;
 written=sprintf(ptr,"PGM / RUN BY / DATE "); ptr+=written; 
 
 padd("** gar2rnx (Garmin to Rinex) generates Rinex2 files",buffer,60);
 written=sprintf(ptr,buffer);  ptr+=written; 
 written=sprintf(ptr,padd("COMMENT",buffer,20)); ptr+=written;

 padd("** from a GPS12 (or XL) (Copyright Antonio Tabernero)",buffer,60);
 written=sprintf(ptr,"%s",buffer);            ptr+=written;
 written=sprintf(ptr,"COMMENT             "); ptr+=written;


 sprintf(buffer,"** Generated from G12 data file: %s ",DATAFILE);
 padd(buffer,buffer,60); 
 written=sprintf(ptr,"%s",buffer); ptr+=written;
 written=sprintf(ptr,"%s",padd("COMMENT",buffer,20)); ptr+=written;

 written=sprintf(ptr,"** Options: "); ptr+=written;
 written=sprintf(ptr,padd(COMMAND_LINE,buffer,60-written)); ptr+=written;
 written=sprintf(ptr,"COMMENT             "); ptr+=written;  

 sprintf(buffer,"%s",marker);
 written=sprintf(ptr,padd(buffer,buffer,60));   ptr+=written; 
 written=sprintf(ptr,padd("MARKER NAME",buffer,20));              ptr+=written;
 written=sprintf(ptr,padd("Number of Antenna marker",buffer,60)); ptr+=written;
 written=sprintf(ptr,padd("MARKER NUMBER",buffer,20));            ptr+=written;
 written=sprintf(ptr,padd("Your name here",buffer,20));           ptr+=written;
 written=sprintf(ptr,padd("Your boss here",buffer,40));           ptr+=written;
 written=sprintf(ptr,"OBSERVER / AGENCY   "); ptr+=written;

 written=sprintf(ptr,"%03d%17c",prod_id,32); 
 if (written>20) written=20; ptr+=written; 
 written=sprintf(ptr,"%s  ",padd(description,buffer,18)); ptr+=20;
 written=sprintf(ptr,"%4.2f%16c",version,32); 
 if (written>20) written=20; ptr+=written;   
 written=sprintf(ptr,"REC # / TYPE / VERS "); ptr+=written;
 

 written=sprintf(ptr,"%d%19c",0,32); ptr+=written;
 written=sprintf(ptr,padd("GPS12 Internal Antenna",buffer,40)); ptr+=written; 
 written=sprintf(ptr,padd("ANT # / TYPE",buffer,20));           ptr+=written; 
 ...
 ptr+=written;
 written=sprintf(ptr,"APPROX POSITION XYZ "); ptr+=written; 

 written=sprintf(ptr,"%14.4f%14.4f%14.4f%18c",0.0,0.0,0.0,32); ptr+=written;
 written=sprintf(ptr,"ANTENNA: DELTA H/E/N"); ptr+=written;
 ...
 written=sprintf(ptr,"%6d",N_OBS); ptr+=written;

 for(k=0,mask=1;k<3;k++,mask*=2) 
	{
     if (OBS_MASK & mask) {written=sprintf(ptr,"%6s",obs[k]);  ptr+=written;} 
	}
 for(k=N_OBS;k<9;k++) {written=sprintf(ptr,"%6c",32);  ptr+=written; }
 written=sprintf(ptr,"# / TYPES OF OBSERV "); ptr+=written; 


 if (RESET_CLOCK) 
  { dt=first_obs-floor(first_obs+0.5); dt=floor(dt*1e9)/1e9; first_obs-=dt; }

 get_current_date(wdays,first_obs,&gmt);
 written=sprintf(ptr,"%6d%6d",1900+gmt.tm_year,gmt.tm_mon+1); ptr+=written; 
 written=sprintf(ptr,"%6d%6d%6d",gmt.tm_mday,gmt.tm_hour,gmt.tm_min); 
 ptr+=written; 
 secs=(gmt.tm_sec)+first_obs-floor(first_obs);
 written=sprintf(ptr,"%12.6f%6cGPS%9c",secs,32,32); ptr+=written; 
 written=sprintf(ptr,"TIME OF FIRST OBS   "); ptr+=written;   

 written=sprintf(ptr,"%6d%54cINTERVAL%12c",INTERVAL,32,32);  ptr+=written; 
 written=sprintf(ptr,"%60cEND OF HEADER       ",32);  ptr+=written;
...
void print_rinex_info(ULONG wdays, double tow,rinex_obs epoch[],FILE *fd)
{
 ...
 if (N_used)  
  {
   if (RESET_CLOCK) { dt=tow-floor(tow+0.5); dt=floor(dt*1e9)/1e9;} else dt=0.0;
...
void add_0x1a_to_epoch(rinex_obs epoch[], type_rec0x1a chan[])
{
 ...
 for(k=0;k<12;k++)
  {
   sv=chan[k].sv; if (sv>=32) continue;
...
void verify_tt(FILE *org)
{
 ...
 for(k=0;k<32;k++) {flag[k]=0xff; tow[k]=-1;}
 ...
 do
 {
  ...
  switch (id)
   {
    case 0x36: 
      rec36=process_0x36(record); sv=rec36.sv; if(sv>32) break;
      if(ONE_SAT && (sv!=SELECTED_SV) ) break;

      cc=rec36.c50; ff=flag[sv];     
      if (ff==0xff) {flag[sv]=0; check[sv][1]=cc; break;}
    ...
    case 0x38: 
      rec=process_0x38(record);  sv=rec.sv; 
      ...
      if (tow[sv]==-1) {tow[sv]=current_tow; break;}
...
type_rec0x38 choose(type_rec0x38 rec[],int nn, rinex_obs last)
{
 ...
 // Only one case
 if (nn==1) { best=rec[0]; return best; }
    ...
    for(k=0;k<nn;k++)
     {
      ...
      if (DIF<dif_min) {dif_min=DIF; best=rec[k];}
     }   
   }
 return best; 
}
...
int verify_c511(type_rec0x38 allrec[],int N,double last_tow,ULONG next_c511)
 {
  ...
  n_511=0; for(j=0;j<40;j++) nn[j]=0;

  for (k=0;k<N;k++)
   {
    cc=allrec[k].c511; test=0;
    for (j=0;j<n_511;j++) if (cc==c511[j]) { nn[j]++; test=1; break;}      
    if (test==0)  { c511[n_511]=cc; nn[n_511]=1; n_511++; }
   } 

  nmax=0;
  for(k=0;k<n_511;k++) if (nn[k]>nmax) { c511_ok=c511[k]; nmax=nn[k]; }
  if (nmax==0) return 0;
...
int remove_duplicates(type_rec0x38 allrec[],int N,rinex_obs epoch[])
 {
  ...
  for(k=0;k<N;k++)
   {
    nn=0;
    rec[nn++]=allrec[k]; sv=rec[0].sv; 
    if (used[sv]<0) continue;        // This sv has already been processed 

    if (used[sv]>1) 
     {
      for(j=k+1;j<N;j++) 
       { if (allrec[j].sv==sv) rec[nn++]=allrec[j]; if (nn==used[sv]) break; } 
      used[sv]=-1;
     }
...
void generate_rinex(FILE *org)
{
 ...
 reset_epoch(epoch);
 for(k=0;k<32;k++) {epoch[k].used=NEVER_USED; epoch[k].last36=-1.0;}
  ...
  switch (id)
   {
    ...
    case 0x16:
      ...
      r16=process_0x16(record); sv=r16.sv; if(sv>=32) break;
    ...
    case 0x38: 
      ...
      rec=process_0x38(record);  sv=rec.sv; if(sv>=32) break;  
      ...
      if (rec.tow==current_tow) allrec[n_records++]=rec; 
      else                             // End of epoch detected
       {
        ...
        nr=n_records; n_records=0;

        //printf("End TOW:  0x38 records: %d  ",nr); 

        // Check that tow is within limits
        itow=(long)floor(current_tow+0.5);  if(START==-1) START=itow;
        dtow=itow-START;
        if( (itow<START) || (itow>LAST) || (dtow>ELAPSED) )  {n_16=0; break;}
        ...
        nr=verify_c511(allrec,nr,last_tow,next_c511); if (nr==0) {n_16=0; break;}
...
void parse_records(FILE *org)
{
 ...
 switch (id)
  {
   case 0x12: process_0x12_gps38(record); break;

   case 0x0e: process_0x0e(record);  break;        
   case 0x11: process_0x11(record);  break;        
   case 0x14: process_0x14(record);  break;        
   case 0x16: process_0x16(record);  break;    
   case 0x1a: process_0x1a(record);  break;	  
   case 0x33: process_0x33(record);  break;
   case 0x36: process_0x36(record);  break;	  
   case 0x37: process_0x37(record);  break;
   case 0x38: process_0x38(record);  break;     
   case 0x39: process_0x39(record);  break;     
   default :
     printf("Record %02x ------------------------------------------\n",id);  
     check(record,L,AS_BYTE); getchar();
     break;  
 }
...
{
  do parse_records(fd); while (feof(fd)==0);
...
typedef struct{\
BYTE prn; double acc; BYTE health; double af[3]; double tgd;\
double crs; double crc; double cuc; double cus; double cic; double cis;\
double M0; double dn; double ecc; double roota;\
double W0; double  Wdot; double i0; double idot; double w;\
double toc; int week; double toe; double tom;\
int iodc; int iode2; int iode3; int last_iode;\
BYTE L2_code; BYTE L2_data; BYTE fit_flag; UINT aodo;\
} EPHEM;     
...
void dump_eph_ok(FILE* fd)
 {
  ...
  if (VC_format) strcpy(f1,"%20.12e"); else strcpy(f1,"%19.12e");
  strcpy(f4,"   "); for(k=0;k<4;k++) strcat(f4,f1); strcat(f4,"\n");


  health=ep.health>>5;  

  // Convert toc time
  week=(ULONG)ep.week; garmin_wdays=(week-521)*7; toc=ep.toc; 
  frac=toc-floor(toc); toc=floor(toc);
  ...
  for(k=0;k<3;k++) ptr+=sprintf(ptr,f1,ep.af[k]); ptr+=sprintf(ptr,"\n");
...
/*
  ...
  */
  ...
  if (VC_format)
  {
   ...
   while(ch=msg[k])
   {
    if (ch=='e') {msg[k-out]='D'; k++; msg[k-out]=msg[k]; k++; out++;}
    else msg[k-out]=ch;
    k++;
   }
   msg[k-out]=0;
...
int format_double_old(char *dest,double a)
 {
  ...
  k=0; while(ptr[k]!='.') k++; pos_coma=k;
  while(ptr[k]!='e') k++; pos_e=k+1;
  exp=atoi(ptr+pos_e);


  first=ptr[pos_coma-1]; sgn=ptr[pos_coma-2];  

  if (sgn=='+') sgn=' '; 

  index=0;
  dest[index++]=' '; dest[index++]=sgn; dest[index++]='.';

  if (first=='0') ND=12; else {dest[index++]=first; ND=11;exp++;}
...
int format_double(char *dest,double a)
 {
  ...
  k=0; while(ptr[k]!='.') k++; pos_coma=k;
  while( (ptr[k]!='e') && (ptr[k]!='E')) k++; pos_e=k+1; exp=atoi(ptr+pos_e);
  ...
  index=0;
  //
  if (leading_cero) {dest[index++]=sgn; dest[index++]='0';}
  else {dest[index++]=' '; dest[index++]=sgn;}
  dest[index++]='.';

  dest[index++]=first; ND=11;exp++;
  for(k=1;k<=ND;k++) dest[index++]=ptr[pos_coma+k];
...
void dump_eph(FILE* fd)
 {
  ...
  for(k=0;k<3;k++) ptr+=format_double(ptr,ep.af[k]); ptr+=sprintf(ptr,"\n");
...
#ifdef TRACE
 printf("D30 %d\n",d30);
 for (k=3;k>=0;k--) printf("%02x ",org[k]); printf("\n");
...
void fill_subframe1()
{
 ...
 strcpy(on_off[0]," ON"); strcpy(on_off[1],"OFF");
 ...
 temp = extrae_ulong(49,10); eph[current_sat].week=temp+1024;  
 temp = extrae_ulong(59,2);  eph[current_sat].L2_code=(BYTE)temp;
 temp = extrae_ulong(61,4);  eph[current_sat].acc=URA_TABLE[temp];
 temp = extrae_ulong(71,2); 
 temp = (temp << 8) + extrae_ulong(169,8);  eph[current_sat].iodc=temp;
 temp = extrae_ulong(73,1);  eph[current_sat].L2_data=(BYTE)temp;
 temp = extrae_ulong(65,6);  eph[current_sat].health=(BYTE)temp;

 L=8; temp = extrae_ulong(161,L); 
 d=get_real(temp,L,1,-31);   eph[current_sat].tgd=d;
 
 temp = extrae_ulong(177,16); temp<<=4; eph[current_sat].toc=temp;
 
 L=22; temp = extrae_ulong(217,L); d=get_real(temp,L,1,-31); 
 eph[current_sat].af[0]=d; 
 L=16; temp = extrae_ulong(201,L); d=get_real(temp,L,1,-43); 
 eph[current_sat].af[1]=d; 
 L=8; temp = extrae_ulong(193,8); d=get_real(temp,L,1,-55);
...
// printf("------------------------------------------------------------------\n");
 ...
 switch(eph[current_sat].L2_code)
  {
   case 0: printf("L2code flag 00 (reserved). "); break;
   case 1: printf("P code ON in L2. "); break;
   case 2: printf("C/A code ON in L2. "); break;
   case 3: printf("L2code flag 11 (?). "); break;
  } 
 ...
 if (eph[current_sat].health)
 {
  ...
  p_nbits(eph[current_sat].health,5); printf("\n");
 }    
 else
...
void fill_subframe2()
{
 ...
 temp = extrae_ulong(49,8);  eph[current_sat].iode2=temp;


 L=16;  temp = extrae_ulong(169,L);  d=get_real(temp,L,1,-29);
 eph[current_sat].cus=d; 
 L=16;  temp = extrae_ulong(121,L);  d=get_real(temp,L,1,-29);
 eph[current_sat].cuc = d; 
 L=16; temp=extrae_ulong(57,L);d=get_real(temp,L,1,-5);
 eph[current_sat].crs=d; 
 
 L=32;  temp = extrae_ulong(89,L);  d=get_real(temp,L,1,-31)*WGS84_PI;
 eph[current_sat].M0 = d; 
 L=16;  temp = extrae_ulong(73,L);  d=get_real(temp,L,1,-43)*WGS84_PI;
 eph[current_sat].dn = d; 
 
 L=32;  temp = extrae_ulong(137,L);  d=get_real(temp,L,0,-33);
 eph[current_sat].ecc=d; 
 L=32;  temp = extrae_ulong(185,L);  d=get_real(temp,L,0,-19);
 eph[current_sat].roota = d;

 L=16;  temp = extrae_ulong(217,L)<<4;  eph[current_sat].toe=temp;

 L=1;  temp = extrae_ulong(233,L);   eph[current_sat].fit_flag=(BYTE)temp;
 L=5;  temp = extrae_ulong(235,L);   eph[current_sat].aodo=temp*900;
 ...
 mu=3.986005e14;  A=pow(eph[current_sat].roota,2.0);
 ...
 n0 = sqrt(mu/pow(A,3.0)); n0=n0+eph[current_sat].dn;
 ...
 if (eph[current_sat].fit_flag==1) printf("> "); printf("4 hours.  ");
...
void fill_subframe3()
{
 ...
 L=16;  temp = extrae_ulong(49,L);  d=get_real(temp,L,1,-29);
 eph[current_sat].cic = d; 
 
 L=32;  temp = extrae_ulong(65,L);  d=get_real(temp,L,1,-31)*WGS84_PI;
 eph[current_sat].W0=d; 
 
 L=16;  temp = extrae_ulong(97,L);  d=get_real(temp,L,1,-29);
 eph[current_sat].cis = d;
 
 L=32;  temp = extrae_ulong(113,L);  d=get_real(temp,L,1,-31)*WGS84_PI;
 eph[current_sat].i0 = d;
 
 L=16;  temp = extrae_ulong(145,L);  d=get_real(temp,L,1,-5);
 eph[current_sat].crc= d;
 
 L=32;  temp = extrae_ulong(161,L);  d=get_real(temp,L,1,-31)*WGS84_PI;
 eph[current_sat].w = d;
 
 L=24;  temp = extrae_ulong(193,L);  d=get_real(temp,L,1,-43)*WGS84_PI;
 eph[current_sat].Wdot = d;
 
 L=8;  temp = extrae_ulong(217,L);  eph[current_sat].iode3=temp;

 L=14;  temp = extrae_ulong(225,L);  d=get_real(temp,L,1,-43)*WGS84_PI;
...
void fill_subframe4()
{
 ...
 sv_id=(BYTE)extrae_ulong(51,6); page=pages[sv_id];
 ...
 switch(page)
 {
  case 0:  printf("Dummy sat. No data\n"); break;
  case 1:  printf("Pages 1,6,11,16 or 21 (repeated). Reserved data\n");
           break;//Reserved
           break; 
  case 12: printf("Pages 12 or 24 (repeated). Reserved data\n"); 
           break;//Reserved
           break; 
  case 13: av=extrae_ulong(57,2);
           switch(av)
               {
           ...
           break; 
  case 14: printf("Page 14: Spare\n"); break; //Spare
           break;//Reserved
  case 15: printf("Page 15: Spare\n"); break; //Spare
           break;//Reserved
  case 17: // Special messages
           for(k=0;k<22;k++) msg[k]=frame[current_sat][7+k]; msg[22]=0;
  ...
  case 18: // IONO  & UTC 
           ...
           printf("%s",tab);
           printf("Alfa: "); 
           for(k=0;k<4;k++) printf("%6.3e ",iono[k]); printf("\n");
           printf("%s",tab);
           printf("Beta: "); 
           for(k=4;k<8;k++) printf("%6.3e ",iono[k]); printf("\n");
          
           printf("       UTC parameters\n");  
           printf("%s",tab);
           L=32;  temp = extrae_ulong(145,L); d=get_real(temp,L,1,-30);
           printf("A0 %.2f nanosec   ",d*1e9); 
           L=24;  temp = extrae_ulong(121,L); d=get_real(temp,L,1,-50);
           printf("A1 %12.5e nanosec/sec\n",d*1e9); 
  
           printf("%s",tab);
           L=8;  temp = extrae_ulong(177,L); temp<<=12;
           printf("Tot %d sec. ",temp); 
           L=8;  temp = extrae_ulong(185,L); printf("WN_t %d weeks. ",temp); 
           L=8;  temp = extrae_ulong(201,L); printf("WN_LSF %d weeks\n",temp); 
          
 
           printf("%s",tab);
           L=8;  temp = extrae_ulong(193,L); d=get_real(temp,L,1,0); 
           printf("DT_LS %.0f sec. ",d); 
           L=8;  temp = extrae_ulong(217,L); d=get_real(temp,L,1,0);
           printf("DT_LSF %.0f sec\n",d); 
           
           printf("%s",tab); 
           L=8;  temp = extrae_ulong(209,L); printf("Day Number %d\n",temp); 

           //getchar();
           break;
  case 19: printf("Page 19. Reserved data\n");
           break;
  case 20: printf("Page 20. Reserved data\n"); 
           break;
  case 22: printf("Page 22. Reserved data\n");
           break;
  case 23: printf("Page 23. Reserved data\n");
           break;

  case 25: printf("Page 25. Sat config for all SVs and health for SVs 25-32\n");
          ...
          for(L=0;L<2;L++)
           { 
            ...
            for(k=16*L;k<16*(L+1);k++) 
             if (sv_conf[k]&0x07) printf(" II "); else  printf("  I ");
            printf("\n");
          } 
         ...
         for(k=24;k<32;k++)
          {
           ...
           bad++;
           printf("\n      PRN %02d (",k+1); detail = sv_health[k] & 0x1f;
...
int show_alm()
 {
  ...
  char *tab="             ";  
   ...
   L=16;  temp = extrae_ulong(57,L);  ecc=get_real(temp,L,0,-21);
   L=8;   temp = extrae_ulong(73,L);  toa=temp<<12;  
   L=16;  temp = extrae_ulong(81,L);  inc=(0.3+get_real(temp,L,1,-19))*WGS84_PI;
   L=16;  temp = extrae_ulong(97,L);  Wdot=get_real(temp,L,1,-38)*WGS84_PI;
   L=8;   temp = extrae_ulong(113,L); health=(BYTE)temp;
   L=24;  temp = extrae_ulong(121,L); roota=get_real(temp,L,0,-11);
   L=24;  temp = extrae_ulong(145,L); W0=get_real(temp,L,1,-23)*WGS84_PI;
   L=24;  temp = extrae_ulong(169,L); w=get_real(temp,L,1,-23)*WGS84_PI;
   L=24;  temp = extrae_ulong(193,L); M0=get_real(temp,L,1,-23)*WGS84_PI;

   L=11;  temp = (extrae_ulong(217,8)<<3) + extrae_ulong(236,3);  
   af0=get_real(temp,L,1,-20);
   
   L=11; temp = extrae_ulong(225,L);  af1=get_real(temp,L,1,-38);

   printf("%s",tab); 
   printf("Almanac Reference Time (toa) %lu sec. ",toa);
   printf("Health "); p_bits(health); printf("\n");
   printf("%s",tab); 
...
void fill_subframe5()
{
 ...
 switch(page)
 {
  case 0:  printf("Dummy sat. Non-available data\n"); break;
  case 25: 
   get_messages(msg);
...
{
 int k,sum;
 sum=0; for (k=0;k<30;k++) sum+=check_frame[current_sat][k];
  ...
  if (type==0)
  ...
  else
   {
    ...
    for (k=0;k<30;k++)
     {
      if ((k%3)==0) printf("  "); 
      p_bits(frame[current_sat][k]); printf("  ");
      if ((k%6)==5) printf("\n"); 
     }
   }
 }
...
BOOLEAN info_frame(ULONG N_frame,char* info_msg)
 ...
 if (preamble!=0x8b) {ptr+=sprintf(ptr,"Bad preamble "); fail=1;}
 if (tow!=N_frame) {ptr+=sprintf(ptr,"Tow missmatch "); fail=1;}
 ...
 sf_id = (BYTE)extrae_ulong(44,3);  ptr+=sprintf(ptr," Subframe %d",sf_id);
...
// printf("SV_ID %d  PAGE %d\n",sv_id,page);
 ...
 switch(sv_id)
  {
   case 0:  ptr+=sprintf(ptr,", page --       ");  break; 
            //ptr+=sprintf(ptr,"Dummy sat:");  break;
   case 57: ptr+=sprintf(ptr,", page %2d*      ",page);  break;
            //ptr+=sprintf(ptr,"(1,6,11,16,21) : ");  break;
   case 62: ptr+=sprintf(ptr,", page %2d*      ",page); break;
            //ptr+=sprintf(ptr,"(12,24)        : "); break;
   default: ptr+=sprintf(ptr,", page %2d       ",page); break;
  } 
 ...
 if (sf_id==4)
  {
   switch(page)
    {
     case 0:  ptr+=sprintf(ptr,"Dummy sat(no data)\n"); break;
     case 1:  ptr+=sprintf(ptr,"Reserved\n"); break;
     case 12: ptr+=sprintf(ptr,"Reserved\n");break;
     case 13:
...
*/
              ...
              break;
     case 14: ptr+=sprintf(ptr,"Spare\n");break;
     case 15: ptr+=sprintf(ptr,"Spare\n");break;
     case 17: ptr+=sprintf(ptr,"ASCII messages\n");break;
     case 18: ptr+=sprintf(ptr,"Iono parameters\n"); break;
     case 19: ptr+=sprintf(ptr,"Reserved\n");break;
     case 20: ptr+=sprintf(ptr,"Reserved\n");break;
     case 22: ptr+=sprintf(ptr,"Reserved\n");break;
     case 23: ptr+=sprintf(ptr,"Reserved\n");break;
     case 25: ptr+=sprintf(ptr,"Sat Config\n"); break;
     default:
 ...
 else
  {
   switch(page)
    {
     case 0:  ptr+=sprintf(ptr,"Dummy sat(no data)\n"); break;
     case -1: ptr+=sprintf(ptr,"\n"); break;
     case 25: ptr+=sprintf(ptr,"Sat 1-24 health data\n"); break;
     default: ptr+=sprintf(ptr,"Almanac (sat %d)\n",sv_id);
    }
  }
...
//    printf("SF_ID %d  FAIL %d\n",sf_id,fail);
	...
	switch (sf_id)
	{
	case 1: fill_subframe1(); break;
	case 2: fill_subframe2(); break;
	case 3: fill_subframe3(); break;
	case 4: fill_subframe4(); break;         
	case 5: fill_subframe5(); break;
	}
...
void reset_eph()
 {
  int k; 
  for(k=0;k<32;k++) 
    {eph[k].iodc=-1; eph[k].iode2=-1; eph[k].iode3=-1; eph[k].last_iode=-1;}
 }
...
int check_info_eph()
 {
  ...
  iodc=eph[current_sat].iodc; iodcc=iodc&0xff; 
    ...
    if ( ( (iodcc!=iode2) | (iode2!=iode3) )  && (iodc!=-1) )
     {
      ...
      printf("%d (%d) %d %d\n",iodc,iodcc,iode2,iode3); getchar();
     }
...
void generate_nav_header(FILE* fd)
{
 ...
 max_lines=15; header=(char*)malloc(max_lines*80*sizeof(char));  
...
void generate_nav(FILE *fd)
{
 ...
 while (feof(fd)==0)
  {
   fread(&id,1,1,fd); fread(&L,1,1,fd); fread(record,1,L,fd); 
   
   if (id==0x36) 
    {
     rec=process_0x36(record);  current_sat=rec.sv;
     if ( current_sat<32)
      {     
...
//Check if stdin is desired
 ...
 if (argc>2) 
  {
   ...
   for(j=3;j<argc;j++) {strcat(COMMAND_LINE," "); strcat(COMMAND_LINE,argv[j]);}
  }
 ...
 SELECTED_SV=0x14;
 ONE_SAT=0;
 SELECTED_RECORDS[0]=0x1a; SELECTED_RECORDS[1]=-1;
 DIF_RECORDS=0;

 ETREX=0;
 RINEX_GENERATION=1;
 VERBOSE=0; VERBOSE_NAV=0;

 ONLY_STATS=0;
 PARSE_RECORDS=0;
 MONITOR_NAV=0; NAV_GENERATION=0;
 VERIFY_TIME_TAGS=0;
 NO_SNR=0;
 ...
 while (arg_num<argc)
  {	
   if (strcmp(argv[arg_num],"-stat")==0) { ONLY_STATS=1; arg_num++;}    
   else if (strcmp(argv[arg_num],"-all")==0) { ONE_SAT=0; arg_num++;}     
   else if (strcmp(argv[arg_num],"-dif")==0) { DIF_RECORDS=1; arg_num++;}     
   else if (strcmp(argv[arg_num],"-reset")==0) { RESET_CLOCK=1; arg_num++;}    
   else if (strcmp(argv[arg_num],"-etrex")==0) { ETREX=1; arg_num++;}  
   else if (strcmp(argv[arg_num],"-parse")==0) 
    { PARSE_RECORDS=1; RINEX_GENERATION=0; VERBOSE=1; arg_num++;}     
   else if (strcmp(argv[arg_num],"-nav")==0) 
    { MONITOR_NAV=0; NAV_GENERATION=1; strcpy(location,"brdc");
      RINEX_GENERATION=0; VERBOSE=0; VERBOSE_NAV=0; arg_num++;} 
   else if (strcmp(argv[arg_num],"-monitor")==0) 
    { 
     MONITOR_NAV=1; RINEX_GENERATION=0;VERBOSE_NAV=VERBOSE=0;
     ONE_SAT=1; SELECTED_SV=atoi(argv[arg_num+1])-1; arg_num+=2;
    }     
   else if (strcmp(argv[arg_num],"-sf")==0) 
    { SELECTED_SF=atoi(argv[arg_num+1]); arg_num+=2; }
   else if (strcmp(argv[arg_num],"-page")==0) 
    { SELECTED_PAGE=atoi(argv[arg_num+1]); arg_num+=2; }
   else if (strcmp(argv[arg_num],"-o1")==0) 
    { OPT1=1; arg_num++; }    
   else if (strcmp(argv[arg_num],"-V")==0) 
    { VERBOSE_NAV=1; arg_num++; }    
   else if (strcmp(argv[arg_num],"-o2")==0) 
    { RELAX=1; arg_num++; }    
   else if (strcmp(argv[arg_num],"-verify")==0) 
    { VERIFY_TIME_TAGS=1; RINEX_GENERATION=0; arg_num++; }    
   else if (strcmp(argv[arg_num],"-rinex")==0) 
    { RINEX_GENERATION=1; VERBOSE=0; arg_num++;}     
   else if (strcmp(argv[arg_num],"-half")==0) 
    { L1_FACTOR=2; arg_num++;}     
   else if (strcmp(argv[arg_num],"-f")==0) 
    { RINEX_FILE=1; arg_num++; }      
   else if (strcmp(argv[arg_num],"-snroff")==0) { NO_SNR=1; arg_num++;}     
   else if (strcmp(argv[arg_num],"-get33")==0) 
    { GET_THIS=atoi(argv[arg_num+1]); arg_num+=2; }      
   else if (strcmp(argv[arg_num],"-start")==0) 
    { START=atoi(argv[arg_num+1]); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-stop")==0) 
    { LAST=atoi(argv[arg_num+1]); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-time")==0) 
    { ELAPSED=atoi(argv[arg_num+1]); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-int")==0) 
    { INTERVAL=atoi(argv[arg_num+1]); arg_num+=2; }                            
   //else if (strcmp(argv[arg_num],"-obs")==0) { N_OBS=atoi(argv[arg_num+1]); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-code")==0) 
	{
	 OBS_MASK = OBS_MASK & 6; arg_num++;
	}  
   else if (strcmp(argv[arg_num],"-phase")==0) 
	{
	 OBS_MASK = OBS_MASK & 5; arg_num++;
	}
   else if (strcmp(argv[arg_num],"-doppler")==0) 
	{
	 OBS_MASK = OBS_MASK & 3; arg_num++;
	}
   else if (strcmp(argv[arg_num],"+code")==0) 
	{
	 OBS_MASK = OBS_MASK | 1; arg_num++;
	}   
    else if (strcmp(argv[arg_num],"+phase")==0) 
	{
	 OBS_MASK = OBS_MASK | 2; arg_num++;
	}      
    else if (strcmp(argv[arg_num],"+doppler")==0) 
	{
	 OBS_MASK = OBS_MASK | 4; arg_num++;
	}   
   else if (strcmp(argv[arg_num],"-area")==0) 
    { strncpy(location,argv[arg_num+1],4); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-mark")==0) 
    { strcpy(marker,argv[arg_num+1]); arg_num+=2; }                            
   else if (strcmp(argv[arg_num],"-sats")==0) 
    { 
     ONE_SAT=0; SELECTED_RECORDS[0]=0x1a;  SELECTED_RECORDS[1]=-1; arg_num++; 
    }    
